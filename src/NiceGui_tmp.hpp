...
void setupActuators(MetrologyResources &met_res) {
  // connect and intialise all piezo stages

  // // Tip/tilt stage 1
  // met_res.tip_tilt_stage1.init();
  // // met_res.tip_tilt_stage1.autozero(); // run autozero if stage does not move
  // met_res.tip_tilt_stage1.move_to_x(0.0f);
  // met_res.tip_tilt_stage1.move_to_y(0.0f);
  // std::this_thread::sleep_for(std::chrono::milliseconds(100));
  // std::cout << "\tPosition: (" << met_res.tip_tilt_stage1.readx() << ", " << met_res.tip_tilt_stage1.ready() << ")
  // urad" << std::endl;

  // // Tip/tilt stage 2
  // met_res.tip_tilt_stage2.init();
  // // met_res.tip_tilt_stage2.autozero(); // run autozero if stage does not move
  // met_res.tip_tilt_stage2.move_to_x(0.0f);
  // met_res.tip_tilt_stage2.move_to_y(0.0f);
  // std::this_thread::sleep_for(std::chrono::milliseconds(100));
  // std::cout << "\tPosition: (" << met_res.tip_tilt_stage2.readx() << ", " << met_res.tip_tilt_stage2.ready() << ")
  // urad" << std::endl;

  // // nF tip/tilt stages
  // met_res.nF_stage_1.init();
  // met_res.nF_stage_1.move_to({0.0, 0.0});
  // std::this_thread::sleep_for(std::chrono::milliseconds(100));
  // auto pos = met_res.nF_stage_1.read();
  // std::cout << "nF Stage 1 Position: " << pos[0] << ", " << pos[1] << std::endl;

  // met_res.nF_stage_2.init();
  // met_res.nF_stage_2.move_to({0.0, 0.0});
  // std::this_thread::sleep_for(std::chrono::milliseconds(100));
  // pos = met_res.nF_stage_2.read();
  // std::cout << "nF Stage 2 Position: " << pos[0] << ", " << pos[1] << std::endl;
}

std::cout << "Starting setupActuators" << std::endl;
NICEcontrol::setupActuators(ressources.metrology);
